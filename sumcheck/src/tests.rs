...
mod test {
    ...
    fn test_sumcheck_protocol() {
        ...
        // NOTE: Constraint is A * B - C
        // 1*2 - 0 = 2. (Other evals ARE zero, but not the 1st)
        let c_evals = vec![
            F::ZERO, // Provoke mismatch so sum isn't zero
            F::from_usize(6),
            F::from_usize(3),
            F::from_usize(20),
            F::from_usize(20),
        ...
        assert_eq!(
            ...
            "Verifier expected_eval did not match independent polynomial eval"
        );
    }
    ...
    fn test_high_degree_sumcheck_protocol() {
        ...
        assert_eq!(
            ...
            "Verifier expected_eval did not match independent multi-constraint evaluation for degree 3"
        );
    }
    ...
    fn test_sumcheck_malicious_claim() {
        ...
        // Malicious claim: Prover tells verifier the sum is 0, but really it's 2
        ...
        // Check that verifier catches & rejects the lie
        assert!(
    ...
    fn test_prover_degree_zero_panic() {
        ...
        // We need to override the degree for this test, but since BadVirtualPoly is hardcoded to 1,
        // let's define another BadVirtualPoly for degree zero.
        #[derive(Debug)]
    ...
    #[test]
    #[should_panic(expected = "virtual polynomial num_vars must stay >= 1 during proof rounds")]
    ...
    fn test_verifier_invalid_degree() {
        ...
        let proof = vec![
            vec![F::ZERO; 3],
            vec![F::ZERO; 2], // Only 2 evals, but degree+1 = 3 expected
        ];
    ...
    fn run_sumcheck_scaling_suite(scales: &[usize]) {
        ...
        for &log_n in scales {
            ...
            // Init pseudo-random eval arrays over boolean hypercube
            ...
            let (_proof, _z) = SumcheckProver::prove::<F, F, _>(
                F::ZERO,
                &mut virtual_poly,
                &mut prover_challenger,
            );
