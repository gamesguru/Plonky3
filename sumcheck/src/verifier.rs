...
impl SumcheckVerifier {
        ...
        Challenger: FieldChallenger<F>,
    {
        ...
        if num_vars > 100 {
            return Err("num_vars too large");
        }
        if degree > 1000 {
            return Err("degree too large");
        }

        let domain_size = degree
            .checked_add(1)
            .ok_or("degree too large (usize overflow)")?;

        // Input validation: check degrees before nested for-loop (quadratic time bugs/exploits?)
        for round_evals in proof {
            if round_evals.len() != domain_size {
                return Err("Invalid univariate polynomial degree in sumcheck round");
            }
        }

        // Bind Fiat–Shamir transcript to public statement before seeing prover's info
